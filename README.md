...
To create these files, execute ``HPQPswd64.exe`` (found in the BCU folder) and save the file (extension *.bin) to the ``\PwdFiles`` folder. 
...
* Passwords need to meet minimum complexity rules (must contain upper and lower case letters and a number) or BIOS will reject the password change but won't issue any specific error message - it will simply return "Invalid password file". The only exception of this rule is an empty password which is always allowed 
...
Each setting is individual changed (not as batch) so if any setting is wrong, it is very clear which setting is causing an issue. BIOS Sledgehammer also tries to “translate” the error code of BCU to an exact explanation as defined by the error code list from the BCU User Guide PDF.
